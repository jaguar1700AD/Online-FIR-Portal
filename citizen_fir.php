    ...
    <span class="navbar-toggler-icon"></span>
		...
		if ($result->num_rows > 0) {
		   	...
		   	while($row = $result->fetch_assoc()) {
		   		if ($row["CPhone"] == $_SESSION["user_phone"]) {
                 if($row["Status"] == 2)
                  {
                  	$isApproved = "Approved";
                  }
		    ...
		    echo "<tr><td>" .$row["FID"]. "</td><td>" . $row["Complaint"] . "</td><td>" . $row["IncidentDate"]. "</td><td>" . $row["IncidentTime"] . "</td><td>" . $row["RegistrationTime"] . "</td><td>".$isApproved."</td><td>"   .'<a href="viewFIR.php?passFID='.$row["FID"].'">View FIR</a>' . "</td></tr>";
			}
